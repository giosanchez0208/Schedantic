...
def main() -> None:
    ...
    w("#")
    w("# RULES: documentation/ANNOTATION_GUIDE.md -- read it once before starting.")
    w("# The short version: tag EVERY temporal expression. Do not delete one")
    w("# because another says the same thing. Redundancy is the normalizer's")
    w("# problem, not yours.")
    w("#")
    w("# Read the > line and decide the spans BEFORE looking at the proposal.")
    w("#")
